...
def lambda_handler(event, context):
    ...
    for station in stations:
        ...
        if len(items) > 0:
            ...
            if AQI_final > 100:
                sendEmail(station, now, main_pollutant, AQI_final)

    return True

def sendEmail(stationID, timestamp, main_pollutant, aqi):
    ses = boto3.client('ses', region_name="us-east-1")
    SENDER = 'ekim390@gatech.edu'
    RECEIVER = 'ekim390@gatech.edu'
    ...
    try:
        response = ses.send_email(
            ...
            Message={
                'Subject': {
                    'Data': 'Air Quality Notification'
                },
                'Body': {
                    'Text': {
                        'Data': 'The Air Quality Index(AQI) for station {} at timestamp {} is {} and has reached a dangerous threshold. The main pollutant is {}.'.format(stationID, timestamp, aqi, main_pollutant)
                    },
                },
            },
