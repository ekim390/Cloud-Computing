...
cursor.execute ("CREATE TABLE `Album` (`albumID` varchar(100) NOT NULL, `name` TEXT NOT NULL, `description` TEXT NOT NULL, `thumbnailURL` TEXT NOT NULL, `createdAt` TIMESTAMP DEFAULT CURRENT_TIMESTAMP, `creator` varchar(100) NOT NULL, PRIMARY KEY (`albumID`), FOREIGN KEY (`creator`) REFERENCES `User` (`email`) ON DELETE CASCADE);")
